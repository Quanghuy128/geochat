...
- [confidence: vừa] Catch-all error trong auth phải phân biệt loại lỗi, không map hết thành cùng 1 message. Pattern: check `error.message.includes("already registered")` trước → message cụ thể; catch-all → message chung "Không thể đăng ký. Vui lòng thử lại." Nếu map nhầm, network error hiển thị "Username đã tồn tại." — gây confuse. **Bối cảnh**: BLOCKER 1 trong review username-auth.
