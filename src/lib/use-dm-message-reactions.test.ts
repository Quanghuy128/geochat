 ...
 *
 * Unit tests cho use-dm-message-reactions.ts — focus: bulk load (1 query, KHÔNG per-message),
 * react() upsert-replace semantics (THINK #3), unreact() 0-row no-op (edge case #4), RLS
 * rejection reverts optimistic state + reactBlockedReason (edge case #1), cancelled-flag
 * race-safety pattern từ đầu (reactions-replies-STATE.md > PLAN).
 */
...
import { useDmMessageReactions } from "./use-dm-message-reactions";

describe("useDmMessageReactions", () => {
  beforeEach(() => {
  ...
  it("bulk-loads reactions for ALL messageIds in ONE query (not per-message)", async () => {
    ...
    const { result } = renderHook(() =>
      useDmMessageReactions(CONVERSATION_ID, { userId: ME_ID }, [MESSAGE_ID, "other-msg"]),
    );
    ...
    expect(summary.reactorUserIds).toEqual([OTHER_ID]);
  });

  it("react(): upserts on conflict (message_id, user_id) — replace semantics, not insert-second-row", async () => {
    const { from, upsertSpy, channel, removeChannel } = makeReactionsFromMock({});
    vi.mocked(createClient).mockReturnValue({ from, channel, removeChannel } as never);

    const { result } = renderHook(() =>
      useDmMessageReactions(CONVERSATION_ID, { userId: ME_ID }, [MESSAGE_ID]),
    );
  ...
  it("react(): optimistic update applies immediately, before server resolves", async () => {
    let resolveUpsert!: (v: { data: null; error: null }) => void;
    const upsertPromise = new Promise<{ data: null; error: null }>((resolve) => {
    ...
    let reactPromise!: Promise<{ error: string | null }>;
    act(() => {
      reactPromise = result.current.react(MESSAGE_ID, "🔥");
    });

    // Optimistic state visible BEFORE server resolves.
    await waitFor(() => {
      expect(result.current.reactionsByMessageId.get(MESSAGE_ID)?.[0]?.emoji).toBe("🔥");
    });
  ...
  it("react(): RLS rejection reverts optimistic state and sets reactBlockedReason", async () => {
    ...
    expect(result.current.reactBlockedReason).toBe("unfriended");
    // Reverted — no lingering optimistic pill.
    expect(result.current.reactionsByMessageId.get(MESSAGE_ID) ?? []).toHaveLength(0);
  });

  it("unreact(): deleting a non-existent reaction (0 rows matched) is a no-op success, not an error", async () => {
  ...
  it("returns empty map when conversationId is null", () => {
    const { from, channel, removeChannel } = makeReactionsFromMock({});
    vi.mocked(createClient).mockReturnValue({ from, channel, removeChannel } as never);

    const { result } = renderHook(() => useDmMessageReactions(null, { userId: ME_ID }, []));
